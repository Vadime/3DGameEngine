...
public class ShadowRenderer {
    ...
    private void update(Window window, Matrix4f viewMatrix, Scene scene) {
        DirectionalLight directionalLight = scene != null ? scene.getDirectionalLight() : null;
        for (int i = 0; i < NUM_CASCADES; i++) {
    ...
    public void render(Window window, Scene scene, ICamera FCamera) {
        update(window, FCamera.getViewMatrix(), scene);
    ...
    private void renderInstancedMeshes(Scene scene) {
        ...
        for (InstancedMesh mesh : mapMeshes.keySet()) {
            ...
            mesh.renderListInstanced(filteredItems, null);
        }
    }
